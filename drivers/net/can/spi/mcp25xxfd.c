...
struct fifos {
	...
	u32 rx_fifo_depth;
	u32 rx_fifo_start;
	/* bitmask of which fifo is a rx fifo */
	u32 rx_fifo_mask;
					...
					struct net_device *net)
{
	...
	if (ret == NETDEV_TX_OK) {
		...
		can_put_echo_skb(skb, priv->net, fifo, frame_len);
	}
...
static int mcp25xxfd_can_probe(struct spi_device *spi)
{
	...
	for (i = 0; i < MCP25XXFD_SPI_TRANSFER_BUFFER_MAX_COUNT; i++) {
		spi_transfer_buffer.spi_tx_kzalloc[i] =
			devm_kzalloc(&spi->dev, MCP25XXFD_BUFFER_TXRX_SIZE, GFP_KERNEL);
		if (!spi_transfer_buffer.spi_tx_kzalloc[i])
			return -ENOMEM;

		spi_transfer_buffer.spi_rx_kzalloc[i] =
			devm_kzalloc(&spi->dev, MCP25XXFD_BUFFER_TXRX_SIZE, GFP_KERNEL);
		if (!spi_transfer_buffer.spi_rx_kzalloc[i])
			return -ENOMEM;
	}
...
static void mcp25xxfd_can_remove(struct spi_device *spi)
{
	struct mcp25xxfd_priv *priv = spi_get_drvdata(spi);
