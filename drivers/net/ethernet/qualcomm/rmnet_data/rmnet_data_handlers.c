					 ...
					 unsigned int skb_size)
{
	...
	if (unlikely(ep->flush_time.tv_sec == 0)) {
		timespec64_to_ns(&ep->flush_time);
		ep->flush_byte_count = 0;
	} else {
		tmp = timespec64_to_ns(&(curr_time));
		curr_time = ns_to_timespec64(tmp);
		diff = timespec64_sub(curr_time, ep->flush_time);
...
static struct napi_struct *get_current_napi_context(void)
{
	...
	(struct sk_buff *skb, struct rmnet_phys_ep_config *config)
{
	...
	if (config->ingress_data_format & RMNET_INGRESS_FORMAT_DEAGGREGATION) {
		...
		while ((skbn = rmnet_data_map_deaggregate(skb, config)) != NULL)
