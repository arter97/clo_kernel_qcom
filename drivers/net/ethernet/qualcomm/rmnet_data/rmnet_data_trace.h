...
DECLARE_EVENT_CLASS
	(rmnet_tc_fc_template,
	 ...
	 TP_STRUCT__entry(__field(u32, handle)
		__field(int, qlen)
		__field(int, enable)
	 ),

	 TP_fast_assign(__entry->handle = tcm_handle;
...
TRACE_EVENT
	(rmnet_start_aggregation,
	 ...
	 TP_STRUCT__entry(__string(name, skb->dev->name)
	 ),

	 TP_fast_assign(__assign_str(name, skb->dev->name);
	 ),
...
TRACE_EVENT
	(rmnet_gro_downlink,
	 ...
	 TP_STRUCT__entry(__field(gro_result_t, res)
	 ),

	 TP_fast_assign(__entry->res = gro_res;
	 ),
