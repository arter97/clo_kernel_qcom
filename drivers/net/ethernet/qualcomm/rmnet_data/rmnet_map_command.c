...
unsigned long rmnet_map_command_stats[RMNET_MAP_COMMAND_ENUM_LENGTH];
			       ...
			       struct rmnet_phys_ep_config *config)
{
	...
	if ((config->ingress_data_format & RMNET_INGRESS_FORMAT_MAP_CKSUMV3) ||
		...
		if (unlikely(skb->len < (sizeof(struct rmnet_map_header_s) +
			...
			rmnet_stats_dl_checksum(RMNET_MAP_CHECKSUM_ERR_BAD_BUFFER);
			return;
		}
					   ...
					   struct rmnet_phys_ep_config *config)
{
	...
	default:
		...
		LOGM("Unknown MAP command: %d", command_name);
