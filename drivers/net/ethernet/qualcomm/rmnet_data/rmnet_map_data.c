...
static void rmnet_map_flush_packet_work(struct work_struct *work)
{
	...
	if (likely(config->agg_state == RMNET_MAP_TXFER_SCHEDULED)) {
		...
		if (likely(config->agg_skb)) {
			...
			config->agg_skb = NULL;
			config->agg_count = 0;
			memset(&config->agg_time, 0, sizeof(struct timespec64));
		}
...
new_packet:
	spin_lock_irqsave(&config->agg_lock, flags);
	memcpy(&last, &config->agg_last, sizeof(struct timespec64));
	timespec64_to_ns(&config->agg_last);

	if (!config->agg_skb) {
		/* Check to see if we should agg first. If the traffic is very
		 * sparse, don't aggregate. We will need to tune this later
		 */
		diff = timespec64_sub(config->agg_last, last);
	 ...
	 struct rmnet_map_dl_checksum_trailer_s *cksum_trailer)
{
	...
	pseudo_checksum = ~ntohs(csum_tcpudp_magic(ip4h->saddr, ip4h->daddr,
		...
		(u16)ip4h->protocol, 0));
	ip_pseudo_payload_checksum =
		rmnet_map_add_checksums(ip_payload_checksum, pseudo_checksum);

	checksum_value_final =
		~rmnet_map_subtract_checksums(ip_pseudo_payload_checksum,
						ntohs(*checksum_field));
	...
	if (unlikely(checksum_value_final == 0)) {
		...
		case IPPROTO_TCP:
			if (*checksum_field == 0xFFFF) {
				LOGD("DL4 Non-RFC compliant TCP checksum found");
				checksum_value_final = ~checksum_value_final;
			}
			break;
		}
	...
	LOGD("DL4 cksum: ~HW: %04X, field: %04X, pseudo header: %04X, final: %04X",
	 ...
	 struct rmnet_map_dl_checksum_trailer_s *cksum_trailer)
{
	...
	ip_pseudo_payload_checksum = rmnet_map_add_checksums(ip_payload_checksum,
							     pseudo_checksum);
	...
	if (unlikely(checksum_value_final == 0)) {
		...
		case IPPROTO_TCP:
			if (*checksum_field == 0xFFFF) {
				LOGD("DL6 Non-RFC compliant TCP checksum found");
				checksum_value_final = ~checksum_value_final;
			}
			break;
		}
	...
	LOGD("DL6 cksum: ~HW: %04X, field: %04X, pseudo header: %04X, final: %04X",
	     ~ntohs(cksum_trailer->checksum_value), ntohs(*checksum_field),
	     pseudo_checksum, checksum_value_final);
 ...
 * @skb:	Pointer to the packet's skb.
 *
 * Generates UL checksum info header for IPv4 and IPv6  over TCP and UDP
	...
	if (skb->ip_summed == CHECKSUM_PARTIAL) {
		...
		if (ip_version == 0x04) {
			rmnet_map_fill_ipv4_packet_ul_checksum_header
				(iphdr, ul_header, skb);
			if (egress_data_format & RMNET_EGRESS_FORMAT_MAP_CKSUMV4)
			...
			rmnet_map_fill_ipv6_packet_ul_checksum_header
				(iphdr, ul_header, skb);
			if (egress_data_format & RMNET_EGRESS_FORMAT_MAP_CKSUMV4)
				rmnet_map_complement_ipv6_txporthdr_csum_field(iphdr);
			ret =  RMNET_MAP_CHECKSUM_OK;
			goto done;
		} else {
