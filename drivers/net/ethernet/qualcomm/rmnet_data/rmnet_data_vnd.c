...
static int rmnet_vnd_ioctl_extended(struct net_device *dev, struct ifreq *ifr)
{
	...
	case RMNET_IOCTL_GET_DRIVER_NAME:
		strscpy(ext_cmd.u.if_name, "rmnet_data",
 ...
 */
static int rmnet_vnd_ioctl(struct net_device *dev, struct ifreq *ifr,
...
static const struct net_device_ops rmnet_data_vnd_ops = {
	...
	.ndo_siocdevprivate = rmnet_vnd_ioctl,
			 ...
			 const char *prefix, int use_name)
{
	...
	dev = alloc_netdev(sizeof(struct rmnet_vnd_private_s),
			   ...
			   rmnet_vnd_setup);
	if (!dev) {
		LOGE("Failed to allocate netdev for id %d", id);
		*new_device = 0;
	...
	if (rc != 0) {
		LOGE("Failed to register netdev [%s]", dev->name);
		free_netdev(dev);
		*new_device = 0;
...
int rmnet_vnd_get_name(int id, char *name, int name_len)
{
	...
	p = strscpy(name, rmnet_devices[id]->name, name_len);
	if (p >= name_len) {
