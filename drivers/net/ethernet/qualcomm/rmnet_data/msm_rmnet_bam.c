...
#define DBG0(x...)  do {					\
		if (msm_rmnet_bam_debug_mask & DEBUG_MASK_LVL0) \
			pr_info(x);				\
} while (0)

#define DBG1(x...)  do {					\
		if (msm_rmnet_bam_debug_mask & DEBUG_MASK_LVL1) \
			pr_info(x);				\
} while (0)

#define DBG2(x...)  do {					\
		if (msm_rmnet_bam_debug_mask & DEBUG_MASK_LVL2) \
			pr_info(x);				\
} while (0)
...
DEVICE_ATTR_RO(wakeups_xmit, 0444, wakeups_xmit_show, NULL);
...
DEVICE_ATTR_RO(wakeups_rcv, 0444, wakeups_rcv_show, NULL);
...
DEVICE_ATTR_RW(timeout, 0644, timeout_show, timeout_store);
#endif

/* Forward declaration */
static int rmnet_ioctl(struct net_device *dev, struct ifreq *ifr,
...
static const struct net_device_ops rmnet_ops_ether = {
	...
	.ndo_siocdevprivate = rmnet_ioctl,
...
static int rmnet_ioctl_extended(struct net_device *dev, struct ifreq *ifr)
{
	...
	case RMNET_IOCTL_GET_DRIVER_NAME:
		strscpy(ext_cmd.u.if_name, RMNET_BAM_DRIVER_NAME,
