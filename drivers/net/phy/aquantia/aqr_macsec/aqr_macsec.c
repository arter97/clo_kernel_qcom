...
static int aqr_mss_mdio_read(struct aqr_port *port, uint16_t mmd, uint16_t addr, uint16_t *data)
{
	*data = port->mdio_ops.aqr_mdio_read(port->priv, mmd, addr);
	return (*data != 0xffff) ? 0 : -ETIME;
}

static int aqr_mss_mdio_write(struct aqr_port *port, uint16_t mmd, uint16_t addr, uint16_t data)
{
	return port->mdio_ops.aqr_mdio_write(port->priv, mmd, addr, data);
}
...
static int
set_raw_ingress_record(struct aqr_port *port, uint16_t *packed_record,
				  ...
				  uint16_t table_index)
{
	...
	for (i = 0; i < num_words; i += 2) {
		aqr_mss_mdio_write(port, MDIO_MMD_VEND1,
	...
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1,
			  ...
			  lut_sel_reg.word_0);
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_INGRESS_LUT_CTL_REGISTER_ADDR,
			  lut_op_reg.word_0);
...
static int
get_raw_ingress_record(struct aqr_port *port, uint16_t *packed_record,
				  u8 num_words, u8 table_id,
				  uint16_t table_index)
{
	struct mss_ingress_lut_addr_ctl_register lut_sel_reg;
	...
	ret = aqr_mss_mdio_write(port, MDIO_MMD_VEND1,
				MSS_INGRESS_LUT_ADDR_CTL_REGISTER_ADDR,
				lut_sel_reg.word_0);
	if (unlikely(ret))
		return ret;
	...
	memset(packed_record, 0, sizeof(uint16_t) * num_words);

	for (i = 0; i < num_words; i += 2) {
		ret = aqr_mss_mdio_read(port, MDIO_MMD_VEND1,
...
static int
set_raw_egress_record(struct aqr_port *port, uint16_t *packed_record,
				 u8 num_words, u8 table_id,
				 uint16_t table_index)
{
	...
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1,
			  ...
			  lut_sel_reg.word_0);
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_EGRESS_LUT_CTL_REGISTER_ADDR,
			  lut_op_reg.word_0);
...
static int
get_raw_egress_record(struct aqr_port *port, uint16_t *packed_record,
				 u8 num_words, u8 table_id,
				 uint16_t table_index)
{
	struct mss_egress_lut_addr_ctl_register lut_sel_reg;
...
int aqr_mss_set_ingress_prectlf_record(struct aqr_port *port,
			   const struct aqr_mss_ingress_prectlf_record *rec,
			   uint16_t table_index)
{
	...
	return set_raw_ingress_record(port, packed_record, 6, 0,
				      ROWOFFSET_INGRESSPRECTLFRECORD +
					      table_index);
}
EXPORT_SYMBOL_GPL(aqr_mss_set_ingress_prectlf_record);

int aqr_mss_get_ingress_prectlf_record(struct aqr_port *port,
				      struct aqr_mss_ingress_prectlf_record *rec,
				      uint16_t table_index)
{
	uint16_t packed_record[6];
	int ret;
	 ...
	 */
	if ((table_index % 2) > 0) {
		ret = get_raw_ingress_record(port, packed_record, 6, 0,
					     ROWOFFSET_INGRESSPRECTLFRECORD +
						     table_index - 1);
		if (unlikely(ret))
			return ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_get_ingress_prectlf_record);

int aqr_mss_set_ingress_preclass_record(struct aqr_port *port,
			    const struct aqr_mss_ingress_preclass_record *rec,
			    uint16_t table_index)
{
	...
	return set_raw_ingress_record(port, packed_record, 20, 1,
				      ROWOFFSET_INGRESSPRECLASSRECORD +
					      table_index);
}
EXPORT_SYMBOL_GPL(aqr_mss_set_ingress_preclass_record);

int aqr_mss_get_ingress_preclass_record(struct aqr_port *port,
			    struct aqr_mss_ingress_preclass_record *rec,
			    uint16_t table_index)
{
	uint16_t packed_record[20];
	int ret;
	 ...
	 */
	if ((table_index % 2) > 0) {
		ret = get_raw_ingress_record(port, packed_record, 20, 1,
					     ROWOFFSET_INGRESSPRECLASSRECORD +
						     table_index - 1);
		if (unlikely(ret))
			return ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_get_ingress_preclass_record);

int aqr_mss_set_ingress_sc_record(struct aqr_port *port,
				 const struct aqr_mss_ingress_sc_record *rec,
				 uint16_t table_index)
{
	...
	return set_raw_ingress_record(port, packed_record, 8, 3,
				      ROWOFFSET_INGRESSSCRECORD + table_index);
}
EXPORT_SYMBOL_GPL(aqr_mss_set_ingress_sc_record);

int aqr_mss_get_ingress_sc_record(struct aqr_port *port,
				 struct aqr_mss_ingress_sc_record *rec,
				 uint16_t table_index)
{
	uint16_t packed_record[8];
	int ret;
	...
	ret = get_raw_ingress_record(port, packed_record, 8, 3,
				     ROWOFFSET_INGRESSSCRECORD + table_index);
	if (unlikely(ret))
		return ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_get_ingress_sc_record);

int aqr_mss_set_ingress_sa_record(struct aqr_port *port,
				 const struct aqr_mss_ingress_sa_record *rec,
				 uint16_t table_index)
{
	uint16_t packed_record[8];
...
EXPORT_SYMBOL_GPL(aqr_mss_set_ingress_sa_record);

int aqr_mss_get_ingress_sa_record(struct aqr_port *port,
				 struct aqr_mss_ingress_sa_record *rec,
				 uint16_t table_index)
{
	uint16_t packed_record[8];
	int ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_get_ingress_sa_record);

int aqr_mss_set_ingress_sakey_record(struct aqr_port *port,
			 const struct aqr_mss_ingress_sakey_record *rec,
			 uint16_t table_index)
{
	...
	case aqr_gen_4:
		packed_record[16] = rec->key_len & 0x7;
		break;
	}


	return set_raw_ingress_record(port, packed_record, 18, 2,
				      ROWOFFSET_INGRESSSAKEYRECORD +
					      table_index);
}
EXPORT_SYMBOL_GPL(aqr_mss_set_ingress_sakey_record);

int aqr_mss_get_ingress_sakey_record(struct aqr_port *port,
				    struct aqr_mss_ingress_sakey_record *rec,
				    uint16_t table_index)
{
	uint16_t packed_record[18];
	int ret;
	...
	ret = get_raw_ingress_record(port, packed_record, 18, 2,
				     ROWOFFSET_INGRESSSAKEYRECORD +
					     table_index);
	if (unlikely(ret))
		return ret;
	...
	case aqr_gen_4:
		rec->key_len = packed_record[16] & 0x7;
		break;
	}

	return 0;
}
EXPORT_SYMBOL_GPL(aqr_mss_get_ingress_sakey_record);

int aqr_mss_set_ingress_postclass_record(struct aqr_port *port,
			     const struct aqr_mss_ingress_postclass_record *rec,
			     uint16_t table_index)
{
	uint16_t packed_record[8];
	...
	return set_raw_ingress_record(port, packed_record, 8, 4,
				      ROWOFFSET_INGRESSPOSTCLASSRECORD +
					      table_index);
}
EXPORT_SYMBOL_GPL(aqr_mss_set_ingress_postclass_record);

int aqr_mss_get_ingress_postclass_record(struct aqr_port *port,
			     struct aqr_mss_ingress_postclass_record *rec,
			     uint16_t table_index)
{
	uint16_t packed_record[8];
	int ret;
	 ...
	 */
	if ((table_index % 2) > 0) {
		ret = get_raw_ingress_record(port, packed_record, 8, 4,
					     ROWOFFSET_INGRESSPOSTCLASSRECORD +
						     table_index - 1);
		if (unlikely(ret))
			return ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_get_ingress_postclass_record);

int aqr_mss_set_ingress_postctlf_record(struct aqr_port *port,
			    const struct aqr_mss_ingress_postctlf_record *rec,
			    uint16_t table_index)
{
	uint16_t packed_record[6];
	...
	return set_raw_ingress_record(port, packed_record, 6, 5,
				      ROWOFFSET_INGRESSPOSTCTLFRECORD +
					      table_index);
}
EXPORT_SYMBOL_GPL(aqr_mss_set_ingress_postctlf_record);

int aqr_mss_get_ingress_postctlf_record(struct aqr_port *port,
			    struct aqr_mss_ingress_postctlf_record *rec,
			    uint16_t table_index)
{
	uint16_t packed_record[6];
	int ret;
	 ...
	 */
	if ((table_index % 2) > 0) {
		ret = get_raw_ingress_record(port, packed_record, 6, 5,
					     ROWOFFSET_INGRESSPOSTCTLFRECORD +
						     table_index - 1);
		if (unlikely(ret))
			return ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_get_ingress_postctlf_record);

int aqr_mss_set_egress_ctlf_record(struct aqr_port *port,
				  const struct aqr_mss_egress_ctlf_record *rec,
				  uint16_t table_index)
{
	uint16_t packed_record[6];
	...
	return set_raw_egress_record(port, packed_record, 6, 0,
				     ROWOFFSET_EGRESSCTLFRECORD + table_index);
}
EXPORT_SYMBOL_GPL(aqr_mss_set_egress_ctlf_record);

int aqr_mss_get_egress_ctlf_record(struct aqr_port *port,
				  struct aqr_mss_egress_ctlf_record *rec,
				  uint16_t table_index)
{
	uint16_t packed_record[6];
	int ret;
	 ...
	 */
	if ((table_index % 2) > 0) {
		ret = get_raw_egress_record(port, packed_record, 6, 0,
...
EXPORT_SYMBOL_GPL(aqr_mss_get_egress_ctlf_record);

int aqr_mss_set_egress_class_record(struct aqr_port *port,
				   const struct aqr_mss_egress_class_record *rec,
				   uint16_t table_index)
{
	...
	return set_raw_egress_record(port, packed_record, 28, 1,
				     ROWOFFSET_EGRESSCLASSRECORD + table_index);
}
EXPORT_SYMBOL_GPL(aqr_mss_set_egress_class_record);

int aqr_mss_get_egress_class_record(struct aqr_port *port,
				   struct aqr_mss_egress_class_record *rec,
				   uint16_t table_index)
{
	uint16_t packed_record[28];
	int ret;
	 ...
	 */
	if ((table_index % 2) > 0) {
		ret = get_raw_egress_record(port, packed_record, 28, 1,
...
EXPORT_SYMBOL_GPL(aqr_mss_get_egress_class_record);

int aqr_mss_set_egress_sc_record(struct aqr_port *port,
				const struct aqr_mss_egress_sc_record *rec,
				uint16_t table_index)
{
	uint16_t packed_record[8];
	...
	case aqr_gen_4:
		packed_record[5] |= (rec->sak_len & 0x7) << 4;
		break;
	}

	packed_record[7] = (rec->valid & 0x1) << 15;

	return set_raw_egress_record(port, packed_record, 8, 2,
				     ROWOFFSET_EGRESSSCRECORD + table_index);
}
EXPORT_SYMBOL_GPL(aqr_mss_set_egress_sc_record);

int aqr_mss_get_egress_sc_record(struct aqr_port *port,
				struct aqr_mss_egress_sc_record *rec,
				uint16_t table_index)
{
	uint16_t packed_record[8];
	int ret;
	...
	ret = get_raw_egress_record(port, packed_record, 8, 2,
				    ROWOFFSET_EGRESSSCRECORD + table_index);
	if (unlikely(ret))
		return ret;
	...
	case aqr_gen_4:
		rec->sak_len = (packed_record[5] >> 4) & 0x7;
		break;
	}
...
EXPORT_SYMBOL_GPL(aqr_mss_get_egress_sc_record);

int aqr_mss_set_egress_sa_record(struct aqr_port *port,
				const struct aqr_mss_egress_sa_record *rec,
				uint16_t table_index)
{
	uint16_t packed_record[12];
	int ret = 0;
	...
	case aqr_gen_2:
		break;
	case aqr_gen_3:
		ret = set_raw_egress_record(port, packed_record, 8, 2,
					ROWOFFSET_EGRESSSARECORD + table_index);
		break;
	case aqr_gen_4:
		packed_record[8] = rec->next_ext_pn & 0xFFFF;
		packed_record[9] = (rec->next_ext_pn >> 16) & 0xFFFF;

		packed_record[10] = rec->ssci & 0xFFFF;
		packed_record[11] = (rec->ssci >> 16) & 0xFFFF;

		ret = set_raw_egress_record(port, packed_record, 12, 2, ROWOFFSET_EGRESSSARECORD + table_index);
	break;
	}

	return ret;
}
EXPORT_SYMBOL_GPL(aqr_mss_set_egress_sa_record);

int aqr_mss_get_egress_sa_record(struct aqr_port *port,
				struct aqr_mss_egress_sa_record *rec,
				uint16_t table_index)
{
	uint16_t packed_record[12];
	int ret = 0;
	...
	case aqr_gen_3:
		...
		break;
	case aqr_gen_4:
		ret = get_raw_egress_record(port, packed_record, 12, 2,
			    ROWOFFSET_EGRESSSARECORD + table_index);
		break;
	}
	...
	case aqr_gen_4:
		rec->next_ext_pn = packed_record[8];
		rec->next_ext_pn |= packed_record[9] << 16;

		rec->ssci = packed_record[10];
		rec->ssci |= packed_record[11] << 16;
	break;
	}

	return ret;
}
EXPORT_SYMBOL_GPL(aqr_mss_get_egress_sa_record);

int aqr_mss_set_egress_sakey_record(struct aqr_port *port,
				   const struct aqr_mss_egress_sakey_record *rec,
				   uint16_t table_index)
{
	...
	ret = set_raw_egress_record(port, packed_record + 8, 8, 2,
				    ROWOFFSET_EGRESSSAKEYRECORD + table_index -
					    32);
	if (unlikely(ret))
		return ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_set_egress_sakey_record);

int aqr_mss_get_egress_sakey_record(struct aqr_port *port,
				   struct aqr_mss_egress_sakey_record *rec,
				   uint16_t table_index)
{
	uint16_t packed_record[16];
	int ret;
	...
	if (unlikely(ret))
		return ret;
	ret = get_raw_egress_record(port, packed_record + 8, 8, 2,
				    ROWOFFSET_EGRESSSAKEYRECORD + table_index -
					    32);
	if (unlikely(ret))
		return ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_get_egress_sakey_record);

int aqr_mss_get_egress_sc_counters(struct aqr_port *port,
				  struct aqr_mss_egress_sc_counters *counters,
				  uint16_t sc_index)
{
	...
	ret = get_raw_egress_record(port, packed_record, 4, 3, sc_index * 8 + 4);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_egress_record(port, packed_record, 4, 3, sc_index * 8 + 5);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_egress_record(port, packed_record, 4, 3, sc_index * 8 + 6);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_egress_record(port, packed_record, 4, 3, sc_index * 8 + 7);
	if (unlikely(ret))
		return ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_get_egress_sc_counters);

int aqr_mss_get_egress_sa_counters(struct aqr_port *port,
				  struct aqr_mss_egress_sa_counters *counters,
				  uint16_t sa_index)
{
	uint16_t packed_record[4];
	int ret;
	...
	ret = get_raw_egress_record(port, packed_record, 4, 3, sa_index * 8 + 0);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_egress_record(port, packed_record, 4, 3, sa_index * 8 + 1);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_egress_record(port, packed_record, 4, 3, sa_index * 8 + 2);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_egress_record(port, packed_record, 4, 3, sa_index * 8 + 3);
	if (unlikely(ret))
		return ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_get_egress_sa_counters);

int aqr_mss_get_egress_common_counters(struct aqr_port *port,
			   struct aqr_mss_egress_common_counters *counters)
{
	uint16_t packed_record[4];
	int ret;
	...
	ret = get_raw_egress_record(port, packed_record, 4, 3, 256 + 0);
	if (unlikely(ret))
		return ret;
	counters->ctl_pkt[0] = packed_record[0] | (packed_record[1] << 16);
	counters->ctl_pkt[1] = packed_record[2] | (packed_record[3] << 16);

	ret = get_raw_egress_record(port, packed_record, 4, 3, 256 + 1);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_egress_record(port, packed_record, 4, 3, 256 + 2);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_egress_record(port, packed_record, 4, 3, 256 + 3);
	if (unlikely(ret))
		return ret;
	counters->too_long[0] = packed_record[0] | (packed_record[1] << 16);
	counters->too_long[1] = packed_record[2] | (packed_record[3] << 16);

	ret = get_raw_egress_record(port, packed_record, 4, 3, 256 + 4);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_egress_record(port, packed_record, 4, 3, 256 + 5);
	if (unlikely(ret))
		return ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_get_egress_common_counters);

int aqr_mss_clear_egress_counters(struct aqr_port *port)
{
	...
	ret = aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_EGRESS_CTL_REGISTER_ADDR,
			       &ctl_reg.word_0);
	if (unlikely(ret))
		return ret;
	ret = aqr_mss_mdio_read(port, MDIO_MMD_VEND1,
...
EXPORT_SYMBOL_GPL(aqr_mss_clear_egress_counters);

int aqr_mss_get_ingress_sa_counters(struct aqr_port *port,
				   struct aqr_mss_ingress_sa_counters *counters,
				   uint16_t sa_index)
{
	uint16_t packed_record[4];
	int ret;
	...
	ret = get_raw_ingress_record(port, packed_record, 4, 6,
				     sa_index * 12 + 0);
	if (unlikely(ret))
		return ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_get_ingress_sa_counters);

int aqr_mss_get_ingress_common_counters(struct aqr_port *port,
			    struct aqr_mss_ingress_common_counters *counters)
{
	uint16_t packed_record[4];
	int ret;
	...
	ret = get_raw_ingress_record(port, packed_record, 4, 6, 385 + 0);
	if (unlikely(ret))
		return ret;
	counters->ctl_pkts[0] = packed_record[0] | (packed_record[1] << 16);
	counters->ctl_pkts[1] = packed_record[2] | (packed_record[3] << 16);

	ret = get_raw_ingress_record(port, packed_record, 4, 6, 385 + 1);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_ingress_record(port, packed_record, 4, 6, 385 + 2);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_ingress_record(port, packed_record, 4, 6, 385 + 8);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_ingress_record(port, packed_record, 4, 6, 385 + 9);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_ingress_record(port, packed_record, 4, 6, 385 + 10);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_ingress_record(port, packed_record, 4, 6, 385 + 11);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_ingress_record(port, packed_record, 4, 6, 385 + 12);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_ingress_record(port, packed_record, 4, 6, 385 + 13);
	if (unlikely(ret))
		return ret;
	...
	ret = get_raw_ingress_record(port, packed_record, 4, 6, 385 + 14);
	if (unlikely(ret))
		return ret;
	counters->ecc_error_pkts[0] =
		packed_record[0] | (packed_record[1] << 16);
	...
	ret = get_raw_ingress_record(port, packed_record, 4, 6, 385 + 15);
	if (unlikely(ret))
		return ret;
	counters->unctrl_hit_drop_redir[0] =
		packed_record[0] | (packed_record[1] << 16);
...
EXPORT_SYMBOL_GPL(aqr_mss_get_ingress_common_counters);

int aqr_mss_clear_ingress_counters(struct aqr_port *port)
{
...
EXPORT_SYMBOL_GPL(aqr_mss_clear_ingress_counters);

int aqr_mss_get_egress_sa_expired(struct aqr_port *port, uint32_t *expired)
{
	uint16_t val;
	int ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_get_egress_sa_expired);

int aqr_mss_get_egress_sa_threshold_expired(struct aqr_port *port,
					   uint32_t *expired)
{
	uint16_t val;
	int ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_get_egress_sa_threshold_expired);

int aqr_mss_set_egress_sa_expired(struct aqr_port *port, uint32_t expired)
{
	int ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_set_egress_sa_expired);

int aqr_mss_set_egress_sa_threshold_expired(struct aqr_port *port, uint32_t expired)
{
	int ret;
...
EXPORT_SYMBOL_GPL(aqr_mss_set_egress_sa_threshold_expired);

/* AQR Gen4 only */
int aqr_mss_set_macsec_mode(struct aqr_port *port, enum aqr_macsec_mode macsec_mode)
{
	uint16_t lowWord, highWord;

	if (port->device != aqr_gen_4)
		return -ENOTSUPP;

	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_EGRESS_CTL_REGISTER_ADDR, &lowWord);

	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_EGRESS_CTL_REGISTER_ADDR + 1, &highWord);
	highWord = (highWord & 0xBFFF) | (macsec_mode << 14);

	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_EGRESS_CTL_REGISTER_ADDR, lowWord);
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_EGRESS_CTL_REGISTER_ADDR + 1, highWord);

	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_EGRESS_CTL_REGISTER_ADDR, &lowWord);
	lowWord = (lowWord & 0x7FFF) | (macsec_mode << 15);
...
EXPORT_SYMBOL_GPL(aqr_mss_set_macsec_mode);

int aqr_mss_get_ingress_salt(struct aqr_port *port, struct aqr_salt_value* salt)
{
	uint16_t lowWord, highWord;
	...
	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_INGRESS_GCM_SALT_REGISTER_ADDR, &lowWord);
	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_INGRESS_GCM_SALT_REGISTER_ADDR + 1, &highWord);
	salt->word0 = (highWord << 16) | lowWord;

	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_INGRESS_GCM_SALT_REGISTER_ADDR + 2, &lowWord);
	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_INGRESS_GCM_SALT_REGISTER_ADDR + 3, &highWord);
	salt->word1 = (highWord << 16) | lowWord;

	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_INGRESS_GCM_SALT_REGISTER_ADDR + 4, &lowWord);
	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_INGRESS_GCM_SALT_REGISTER_ADDR + 5, &highWord);
	salt->word2 = (highWord << 16) | lowWord;

	  return 0;
}
EXPORT_SYMBOL_GPL(aqr_mss_get_ingress_salt);

int aqr_mss_set_ingress_salt(struct aqr_port *port, const struct aqr_salt_value* salt)
{
	uint16_t val;
	...
	val = salt->word0 & 0xFFFF;
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_INGRESS_GCM_SALT_REGISTER_ADDR, val);

	val = salt->word0 >> 16;
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_INGRESS_GCM_SALT_REGISTER_ADDR + 1, val);

	val = salt->word1 & 0xFFFF;
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_INGRESS_GCM_SALT_REGISTER_ADDR + 2, val);

	val = salt->word1 >> 16;
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_INGRESS_GCM_SALT_REGISTER_ADDR + 3, val);

	val = salt->word2 & 0xFFFF;
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_INGRESS_GCM_SALT_REGISTER_ADDR + 4, val);

	val = salt->word2 >> 16;
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_INGRESS_GCM_SALT_REGISTER_ADDR + 5, val);

	return 0;
}
EXPORT_SYMBOL_GPL(aqr_mss_set_ingress_salt);

int aqr_mss_get_egress_salt(struct aqr_port *port, struct aqr_salt_value* salt)
{
	...
	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_EGRESS_GCM_SALT_REGISTER_ADDR, &lowByte);
	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_EGRESS_GCM_SALT_REGISTER_ADDR + 1, &highByte);
	salt->word0 = (highByte << 16) | lowByte;

	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_EGRESS_GCM_SALT_REGISTER_ADDR + 2, &lowByte);
	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_EGRESS_GCM_SALT_REGISTER_ADDR + 3, &highByte);
	salt->word1 = (highByte << 16) | lowByte;

	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_EGRESS_GCM_SALT_REGISTER_ADDR + 4, &lowByte);
	aqr_mss_mdio_read(port, MDIO_MMD_VEND1, MSS_EGRESS_GCM_SALT_REGISTER_ADDR + 5, &highByte);
	salt->word2 = (highByte << 16) | lowByte;

	return 0;
}
EXPORT_SYMBOL_GPL(aqr_mss_get_egress_salt);

int aqr_mss_set_egress_salt(struct aqr_port *port, const struct aqr_salt_value* salt)
{
	uint16_t val;
	...
	val = salt->word0 & 0xFFFF;
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_EGRESS_GCM_SALT_REGISTER_ADDR, val);

	val = salt->word0 >> 16;
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_EGRESS_GCM_SALT_REGISTER_ADDR + 1, val);

	val = salt->word1 & 0xFFFF;
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_EGRESS_GCM_SALT_REGISTER_ADDR + 2, val);

	val = salt->word1 >> 16;
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_EGRESS_GCM_SALT_REGISTER_ADDR + 3, val);

	val = salt->word2 & 0xFFFF;
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_EGRESS_GCM_SALT_REGISTER_ADDR + 4, val);

	val = salt->word2 >> 16;
	aqr_mss_mdio_write(port, MDIO_MMD_VEND1, MSS_EGRESS_GCM_SALT_REGISTER_ADDR + 5, val);

	return 0;
}
EXPORT_SYMBOL_GPL(aqr_mss_set_egress_salt);

MODULE_DESCRIPTION("Aquantia PHY MACSEC driver");
MODULE_AUTHOR("Pavel Belous <pbelous@marvell.com>");
