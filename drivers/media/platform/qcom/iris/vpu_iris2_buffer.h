...
#ifndef _VPU_IRIS2_BUFFER_H_
#define _VPU_IRIS2_BUFFER_H_
			  ...
			  u32 _yuv_bufcount_min)
{
	...
	return (size_colloc * (_yuv_bufcount_min)) +
...
u32 size_h264d_bse_cmd_buf(u32 frame_width, u32 frame_height)
{
	...
	return min_t(u32, (((_height + 15) >> 4) * 3 * 4), H264D_MAX_SLICE) *
...
u32 size_h264d_vpp_cmd_buf(u32 frame_width, u32 frame_height)
{
	...
	_size = min_t(u32, (((_height + 15) >> 4) * 3 * 4), H264D_MAX_SLICE) *
...
static inline u32 hfi_iris2_vp9d_comv_size(void)
{
...
static inline
u32 hfi_iris2_vp9d_lb_size(u32 frame_width, u32 frame_height,
			   u32 num_vpp_pipes)
{
			 ...
			 u32 num_vpp_pipes)
{
	...
	_lb_size = hfi_iris2_vp9d_lb_size(frame_width, frame_height,
					  num_vpp_pipes);
...
static inline u32 hfi_buffer_persist_vp9d(void)
{
	return ALIGN(VP9_NUM_PROBABILITY_TABLE_BUF * VP9_PROB_TABLE_SIZE,
		     DMA_ALIGNMENT) +
		ALIGN(hfi_iris2_vp9d_comv_size(), DMA_ALIGNMENT) +
...
static inline
u32 hfi_buffer_bitstream_enc_iris2(u32 frame_width, u32 frame_height,
...
static inline
u32 hfi_iris2_enc_recon_buf_count(s32 n_bframe, s32 ltr_count,
...
u32 hfi_buffer_bin_enc(u32 rc_type, u32 frame_width, u32 frame_height, u32 lcu_size,
		       u32 work_mode, u32 num_vpp_pipes)
{
	...
	if (work_mode == HFI_WORKMODE_2) {
		...
		bitbin_size = ALIGN(bitbin_size, DMA_ALIGNMENT);
...
{
	return ALIGN((256 + (num_lcu_in_frame << 4)), DMA_ALIGNMENT);
}

static inline
u32 size_line_buf_ctrl(u32 frame_width_coded)
{
	return ALIGN(frame_width_coded, DMA_ALIGNMENT);
}
...
u32 size_linebuff_data(bool is_ten_bit, u32 frame_width_coded)
{
	...
	_size = is_ten_bit ?
		(((((10 * (frame_width_coded) + 1024) + (DMA_ALIGNMENT - 1)) &
		   (~(DMA_ALIGNMENT - 1))) * 1) +
		 (((((10 * (frame_width_coded) + 1024) >> 1) + (DMA_ALIGNMENT - 1)) &
		   (~(DMA_ALIGNMENT - 1))) * 2)) :
		(((((8 * (frame_width_coded) + 1024) + (DMA_ALIGNMENT - 1)) &
		   (~(DMA_ALIGNMENT - 1))) * 1) +
		 (((((8 * (frame_width_coded) + 1024) >> 1) + (DMA_ALIGNMENT - 1)) &
		   (~(DMA_ALIGNMENT - 1))) * 2));
					...
					u32 num_vpp_pipes_enc)
{
	...
	_size = ((DMA_ALIGNMENT + 64 * ((frame_height_coded) /
		  (8 * (is_ten_bit ? 4 : 8)))));
	_size = ALIGN(_size, DMA_ALIGNMENT);
...
{
	return ALIGN((256 + (4 * (num_lcu_in_frame))), DMA_ALIGNMENT);
}

static inline u32 enc_bitmap_buf_size(u32 num_lcu_in_frame)
{
	return ALIGN((256 + ((num_lcu_in_frame) >> 3)), DMA_ALIGNMENT);
}

static inline u32 size_line_buf_sde(u32 frame_width_coded)
{
	return ALIGN((256 + (16 * ((frame_width_coded) >> 4))), DMA_ALIGNMENT);
}

static inline u32 size_override_buf(u32 num_lcumb)
{
	return ALIGN(((16 * (((num_lcumb) + 7) >> 3))), DMA_ALIGNMENT) * 2;
}

static inline u32 size_ir_buf(u32 num_lcu_in_frame)
{
	return ALIGN((((((num_lcu_in_frame) << 1) + 7) & (~7)) * 3), DMA_ALIGNMENT);
}
...
{
	return ALIGN((16 * ((frame_width_coded) >> 5)), DMA_ALIGNMENT);
}
...
static inline
u32 hfi_iris2_enc_min_input_buf_count(u32 totalhblayers)
{
	u32 numinput = 3;
