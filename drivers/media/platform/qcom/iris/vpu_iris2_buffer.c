...
static u32 dec_bin_size_iris2(struct iris_inst *inst)
{
	...
	if (inst->codec == H264)
		size = hfi_buffer_bin_h264d(width, height, DEFAULT_BSE_VPP_DELAY, num_vpp_pipes);
	else if (inst->codec == HEVC)
		size = hfi_buffer_bin_h265d(width, height, DEFAULT_BSE_VPP_DELAY, num_vpp_pipes);
...
static u32 dec_comv_size_iris2(struct iris_inst *inst)
{
...
static u32 dec_non_comv_size_iris2(struct iris_inst *inst)
{
	u32 width, height, num_vpp_pipes;
	struct iris_core *core;
	struct v4l2_format *f;
...
static u32 dec_line_size_iris2(struct iris_inst *inst)
{
...
static u32 dec_persist_size_iris2(struct iris_inst *inst)
{
	u32 size = 0;
...
static u32 dec_dpb_size_iris2(struct iris_inst *inst)
{
	struct v4l2_format *f;
	u32 width, height;
	u32 color_fmt;
...
static u32 enc_bin_size_iris2(struct iris_inst *inst)
{
...
u32 get_recon_buf_count_iris2(struct iris_inst *inst)
{
	...
	num_buf_recon = hfi_iris2_enc_recon_buf_count(n_bframe, ltr_count,
						      hp_layers, hb_layers,
...
static u32 enc_comv_size_iris2(struct iris_inst *inst)
{
	...
	num_recon = get_recon_buf_count_iris2(inst);
	if (inst->codec == H264)
...
static u32 enc_non_comv_size_iris2(struct iris_inst *inst)
{
	u32 width, height, num_vpp_pipes;
	struct iris_core *core;
	struct v4l2_format *f;
...
static u32 enc_line_size_iris2(struct iris_inst *inst)
{
...
static u32 enc_dpb_size_iris2(struct iris_inst *inst)
{
...
static u32 enc_arp_size_iris2(struct iris_inst *inst)
{
	u32 size = 0;
...
static u32 enc_vpss_size_iris2(struct iris_inst *inst)
{
...
u32 enc_output_buffer_size_iris2(struct iris_inst *inst)
{
	...
	frame_size = hfi_buffer_bitstream_enc_iris2(f->fmt.pix_mp.width,
						    f->fmt.pix_mp.height,
...
int iris_int_buf_size_iris2(struct iris_inst *inst,
			    enum iris_buffer_type buffer_type)
{
	...
	static const struct iris_buf_type_handle dec_internal_buf_type_handle[] = {
		{BUF_BIN,             dec_bin_size_iris2          },
		{BUF_COMV,            dec_comv_size_iris2         },
		{BUF_NON_COMV,        dec_non_comv_size_iris2     },
		{BUF_LINE,            dec_line_size_iris2         },
		{BUF_PERSIST,         dec_persist_size_iris2      },
		{BUF_DPB,             dec_dpb_size_iris2          },
	};
	static const struct iris_buf_type_handle enc_internal_buf_type_handle[] = {
		{BUF_BIN,             enc_bin_size_iris2          },
		{BUF_COMV,            enc_comv_size_iris2         },
		{BUF_NON_COMV,        enc_non_comv_size_iris2     },
		{BUF_LINE,            enc_line_size_iris2         },
		{BUF_DPB,             enc_dpb_size_iris2          },
		{BUF_ARP,             enc_arp_size_iris2          },
		{BUF_VPSS,            enc_vpss_size_iris2         },
	};
